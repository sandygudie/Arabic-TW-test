...
(rr-testing-challenges-stock-code)=
## 测试恒星代码
...
def my_function():
  ...
  b = calculation_that_uses_fiv_random_numers()
  c = a + b
```
...
- 变异式：CU_ASSERT_DOUBLE_EQUAL(包裹，每秒)
- CPPUnit for C++: CPPUNT_ASSERT_DOUBLES_EQUAL(assum, actual, delta)
- C++的谷歌测试：ASSERT_NEAR(val1, val2, abs_error)
- FRUIT for Fortran: subregular assert_eq_double_in_range_(var1, var2, delta, message)
- Java JUnit : org.junit.Assert.sertEquals(signedsixix jetics, sedsixybutches siximent, xycyclear)
- 测试为 R：
