...
(rr-testing-challenges-hardatify)=
## 定量化が困難なテスト
...
- C单元：CU_ASSERT_DOUBLE_EQUAL(实际，预期颗粒)
- C+readCPPPUnit: CPPUNT_ASSERT_DOUBLES_EQUAL(需要, 实际, 三角洲)
- C++ 双精度： ASSERT_NEAR(val1, val2, abs_error)
- FRUIT for Fortran: sub-regular assert_eq_double_in_range_(var1, var2, delta, message)
- JUnit for Java : org.junit.Assert.assertEquals(双倍预期、双倍实际、双倍增益)
- Rのテスト:
  - 预期等价(实际，预期耐受=DELTA) - DELTADE
  - 期望等价(实际，预期，尺寸=预期，容忍度=DELTA) - DEL T
