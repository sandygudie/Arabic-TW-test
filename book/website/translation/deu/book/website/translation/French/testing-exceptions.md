...
```python
def my_function() :
  eine = calculation_that_uses_two_random_numbers()
...
## Tests Difficiles a<unk> Quantifier
...
De nombreux frameworks de test fournissent des fonctions permettant de comparer l'égalité des nombres à virgule flottante à une tolérance donnée. Zum Beispiel für den Pytest des Frameworks :
...
- googletest pour C++ : ASSERT_NEAR(val1, val2, Abwesenheit)
- FRUIT pour Fortran: Unterroutine assert_eq_double_in_range_(var1, var2, delta, message)
- JUnit pour Java : org.junit.Assert.assertEquals(double attendu, double re<unk> el, double delta)
- testthat pour R:
